...
if (reg1.test($request.url)) {
  ...
  if ($prefs.valueForKey("request_id") !== null) {
    $notify("Auto TestFlight", "Đã lấy thông tin", "");
  } else {
    $notify("Auto TestFlight", "Không thể lấy thông tin", "Cần thêm testflight.apple.com");
  }
  $done({});
  ...
  $notify("Auto TestFlight", `Đã thêm APP_ID: ${id}`, `ID còn lại: ${appId}`);
  $done({});
}
